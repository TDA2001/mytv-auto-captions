...
docker buildx build --platform linux/amd64 --push -q -t $ECR_REGISTRY/$STREAM_REPOSITORY_NAME:latest .
if [ $? != 0 ]; then exit 1; fi
...
docker buildx build --platform linux/amd64 --push -q -t $ECR_REGISTRY/$TRANSCRIBE_REPOSITORY_NAME:latest .
rm -rf ./src/utils
if [ $? != 0 ]; then exit 1; fi
...
then 
	...
	docker buildx build --platform linux/amd64 --push -q -t $ECR_REGISTRY/$TRANSLATE_REPOSITORY_NAME:latest .
	rm -rf ./src/utils
	cd ../../deployment
	if [ $? != 0 ]; then exit 1; fi
fi
